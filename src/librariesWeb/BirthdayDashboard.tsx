...
import { Container,Stack, ToggleButton, Grid,IconButton, Popover , Tooltip, Avatar,ToggleButtonGroup, Card, CardHeader, Box, Badge, Typography } from '@mui/material';
  ...
  return (
    <Card sx={{height:"280px"}}>
     <Grid container sx={{mb:"-7px"}}>
        <Grid item xs={6}>
        <Typography variant='h3' p={1} sx={{color:"#304ffe"}}>Birthdays</Typography>
        </Grid>
       
        <Grid item xs={6} >
        <Box sx={{display:"flex" , justifyContent:"flex-end" }}>
        <RefreshIcon onClick={() => { setIsRefresh(!isRefresh) }} sx={{mt:"11px" , mr:"5px"}} />
          <IconButton sx={{ mt:"5px"}}>
          <Badge badgeContent={Birthdays.length !== 0 ? Birthdays.length : '0'} color="secondary"/>
          </IconButton>
        <IconButton sx={{ mt:"5px"}}>
