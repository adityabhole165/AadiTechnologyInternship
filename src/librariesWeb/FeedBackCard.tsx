...
import { Card, Container , Grid, Stack ,CircularProgress, IconButton, Avatar} from '@mui/material';
import { IGetUserFeedbackBody } from 'src/interfaces/Student/IFeedback';
import { useNavigate } from 'react-router-dom';
import { Typography , Box, Divider ,Popover ,Tooltip ,List} from '@mui/material';
...
const open = Boolean(anchorEl);
  return (
    <Card sx={{height:"370px",overflow: "auto"}}>
        ...
        <Grid item xs={6}>
        <Typography variant="h3" p={1} sx={{color:"#304ffe"}}>Feedback</Typography>
        </Grid>
        <Grid item xs={6}>
        <Box sx={{display:"flex" , justifyContent:"flex-end"}}>
            <RefreshIcon sx={{mr:"1px" , mt:"10px"}} onClick={clickRefresh}/>
        <Avatar sx={{height:"20px" , width:"20px" ,mt:"13px" , mr:"10px"}} src={'/imges/arrow.png'} onClick={ClickFeddback} />
        </Box>
        </Grid>
        </Grid>
      {loading ? (
      ...
      <>
        ...
        <Box key={i}>
           <Grid container>
        <Grid item xs={0.5} >
        <AccessTimeIcon sx={{ml:"10px", color:"#64b5f6"}} fontSize='small'/>
       </Grid>
        <Grid item xs={11.5}>
        ...
        <Typography variant="h5">{item.Header}</Typography>
         <Typography variant="body2" >{item.Text2}</Typography>
           ...
           <Typography variant="body2" px={3} sx={{overflow:"hidden" ,whiteSpace:"nowrap" ,textOverflow:"ellipsis", width:"300px"}} 
           >{item.Text3 }</Typography>
          </Tooltip>
        </Grid>
         </Grid>
