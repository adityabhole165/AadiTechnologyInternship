...
export const getClassPassFailDetailsForTest =
  (data: IGetClassPassFailDetailsForTestBody): AppThunk =>
    async (dispatch) => {
      ...
      let IsSubmitted = response.data?.LstGetFileDetails.length>0?
      response.data?.LstGetFileDetails[0].Is_Submitted:"N";
      ...
      response.data?.LstGetFileDetails.map((columnItem, i) => {
        ...
        response.data?.LstExamStatusForTest.map((headerItem) => {
          row.push(GetItem(headerItem.ExamStatusSortOrder, columnItem.Subject_Id))
        })
        Column.push(row)
      })
