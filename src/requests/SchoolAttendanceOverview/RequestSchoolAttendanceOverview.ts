...
export const GetStudentAttendance =
  (data: IGetSchoolAttendanceOverviewBody): AppThunk =>
    async (dispatch) => {
      ...
      let GridData = [], GridLastData = [""], RowData = [], i = 0, j = 0, PresentStudentWithTotal = ""
      for (j = 0; j < StdArray.length; j++) {
        RowData = [StdArray[j]]
        for (i = 0; i < DivArray.length; i++) {
          PresentStudentWithTotal = getPresentStudentWithTotal(DivArray[i], StdArray[j])
      ...
      GridLastData.push(  "Total-"+response.data.Marked_Total_DivisionList[0].MarkedDivision_TotalDivision)
      GridLastData.push(response.data.Present_Total_StudentList[0].PresentStudent_TotalStudent)
      GridLastData.push(response.data.PresentStudent_PercentageList[0].PresentStudentPercentage)
        GridData.push(GridLastData)
        dispatch(SchoolAttendanceOverviewSlice.actions.RAttendanceOverviewGridData(GridData))
      dispatch(SchoolAttendanceOverviewSlice.actions.RAttendanceOverviewDivArray(DivArray))
      dispatch(SchoolAttendanceOverviewSlice.actions.GetWeekendStatusList(response.data.WeekendStatusList[0].OutSideAcademicYear))
    }
