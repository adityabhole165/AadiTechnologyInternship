...
export const getDeleteMessagePermantely =
  ...
  async (dispatch) => {
    ...
    const response =
      await ApiDeleteMessagePermanently.DeleteMessagePermanentlyapi(data);
    dispatch(
      SliceDeleteMessagePermanetly.actions.GetDeleteMessagePermantely(
        response.data
      )
    );
  };
...
export const getUnDeleteMessages =
  ...
  async (dispatch) => {
    dispatch(SliceDeleteMessagePermanetly.actions.getLoading(true));
    const response = await ApiDeleteMessagePermanently.UnDeleteMessagesapi(
      data
    );
    dispatch(
      SliceDeleteMessagePermanetly.actions.GetDeleteMessagePermantely(
