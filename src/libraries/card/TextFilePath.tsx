import { TextField, Box, Grid, Typography, Stack, IconButton } from '@mui/material'
...
function TextFilePath({ item, onFileSelect, onTextChange }) {
  const  width = 112, height = 151, maxFileSize = 80000
  ...
  const changeFile = async (e) => {
    let isValid = CheckFileValidationUploadPic(e.target.files[0], arrAllowedFiles, maxFileSize)
    setError(isValid)
    if (isValid === null) {
  ...
  return (
    <div>
      ...
      <Box sx={{ textAlign: "center", mt: "10px" }}>
        <img id="imgParent" width="112" height="151" style={{ border: "1px solid gray" }}
