            ...
            <TableContainer component={Paper}>
                ...
                <TableHead>
                    <TableRow  sx={{backgroundColor:"lightgrey"}}>
                        <Checkbox checked={getIsParentCheckedAll()} 
                        onChange={(e)=>{(CheckParentAll(e.target.checked))}}/>
                        Select All
                    </TableRow>
                </TableHead>
                    <TableBody>
                            ...
                            return (
                                <TableRow key={index}>
                                    <TableCell align="center" >
                                        <Checkbox checked={getIsCheckedAll(ParentItem.Id)}
                                            onChange={() => { ClickParentCheckbox(ParentItem.Id) }}>
                                        </Checkbox>
                                        {ParentItem.Name}
                                    </TableCell>
                                    {ItemList.filter((obj) => { return obj.ParentId == ParentItem.Id })
                                        .map((item, index) => {
                                            return (<TableCell align="center" key={index}>
                                                <Checkbox checked={item.IsActive}
                                                    onChange={() => { ClickChildCheckbox(item.Id) }}>
                                                </Checkbox>
                                                {item.Name}
                                            </TableCell>
