...
const ChooseFiles = ({ validFiles, onChangeFiles, maxfileSize = 0, multipleAllowed = true }) => {
    ...
    const fileChangedHandler = async (multipleFiles) => {
        for (let i = 0; i < multipleFiles.length; i++) {
            ...
            if (isValid) {
                ...
                setFileError('')
                setFinalBase642New(prev => [...prev, { FileName: multipleFiles[i].name, Base64URL: multipleFiles[i] }])
                onChangeFiles([...finalBase642New, { FileName: multipleFiles[i].name, Base64URL: multipleFiles[i] }])
            }
        }
    };
    ...
    const handleRemoveListItems = (fileName) => {
        setFinalBase642New((item) => item.filter((obj) => obj.FileName !== fileName))
        onChangeFiles(finalBase642New.filter((obj) => obj.FileName !== fileName))
        aRef.current.value = null;
    }

    return (<>
        Upload files<CloudUploadIcon onClick={handleClick} />

        <input ref={aRef} multiple={multipleAllowed} type="file" style={{ display: 'none' }}
        ...
        {finalBase642New.length > 0 &&
            (< div style={{ marginTop: '10px' }}>
                ...
                {finalBase642New.map((item, i) => {
                    return (<Box key={i}><Grid container>
                        <Grid item xs={2}>
                            ...
                            <IconButton edge="end" aria-label="delete" title="Delete"
                                ...
                                <DeleteIcon sx={{ color: 'red', mr: '-50px', mt: '-8px' }} />
                            </IconButton>
                        </Grid>
                    </Grid></Box>)
                })}
