import { Box, Container, FormControlLabel, Grid, Stack, TextField, Typography } from '@mui/material';
...
import { GetAllClassAndDivision, GetDeleteEvent, GetDeleteEventImagee, GetEventdetail, GetEventtList, GetSelectedStandardAndDivisionCheckBoxx, GetupdateEvent, ResetDeletedLog, resetMessage } from 'src/requests/EventManegment/RequestEventManegment';
...
const EventsManagement = () => {
    ...
    const SvaeUpdateeEvent: any = useSelector((state: RootState) => state.EventsManagement.SaveUpdateEventt);
    ...
    const getEventString = () => {
        ...
        if (Array.isArray(ItemList)) {
            ItemList.map((item) => {
                if (item.IsActive)
                    XMLString = XMLString + " <Division StandardDivisionId=\"" + item.Id + "\" />"
            })
        }
    ...
    const filePath = localStorage.getItem('SiteURL') + '/RITeSchool/' +
