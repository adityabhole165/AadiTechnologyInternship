      ...
      <TableContainer>
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              <TableRow>
                <TableCell colSpan={columns.length} align="center" sx={{ paddingTop: '5px', paddingBottom: '5px' }}>
                  <CircularProgress />
                </TableCell>
              </TableRow>
            ) : data.length === 0 ? (
              ...
              isPagination ? (
                ...
                data.map((row, rowIndex) => (
                  <TableRow key={rowIndex}>
                    {columns.map((column) => (
                      <TableCell {...column.cellProps} key={column.id} sx={{ paddingTop: '5px', paddingBottom: '5px' }}>{column.renderCell(row)}</TableCell>
                    ))}
                  </TableRow>
                ))
              )
